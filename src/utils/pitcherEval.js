...
export function evaluateRookie(scouting) {
  ...
  const EDU_CONFIG = {
    '고교 리그': { scale: 0.30, baseBonus: 0 },  // 미검증, 잠재력 중심
    '독립 리그': { scale: 0.37, baseBonus: 1 },  // 실전 경험 있으나 수준 낮음
    '대학 리그': { scale: 0.45, baseBonus: 2 },  // 즉전감, 스카우팅 신뢰도 높음
    'KBO_FARM':  { scale: 0.52, baseBonus: 4 },  // 퓨처스 경험, 프로 적응 완료
  };
  const { scale: ROOKIE_SCALE, baseBonus } = EDU_CONFIG[education] ?? EDU_CONFIG['고교 리그'];
